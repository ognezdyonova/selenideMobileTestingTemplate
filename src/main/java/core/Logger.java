...
public class Logger extends TestListenerAdapter {
    ...
    public void onTestStart(ITestResult result) {
        ...
        testClassName = null;
        ConsoleLogger.log.info("[" + getCurrentTime() + "] " + "Test " + result.getName() + " is start");
    ...
    public void onTestSuccess(ITestResult tr) {
        ...
        ConsoleLogger.log.info("[+] " + tr.getName() + ": is success" + " [" + getCurrentTime() + "]");
        ...
        if (Boolean.parseBoolean(Prop.getField("bs.available"))) {
            this.markSession(PASSED, "All cases passed");
        }
    }
    ...
    public void onTestFailure(ITestResult tr) {
        ...
        ConsoleLogger.log.info("[-] " + tr.getName() + ": is fail" + " [" + getCurrentTime() + "]");
        endTime = System.currentTimeMillis();
        setTimeStamp(tr.getName(), endTime - startTime);
        startTime = 0;
        endTime = 0;
        failedCase = tr.getName();
        if (Boolean.parseBoolean(Prop.getField("bs.available"))) {
            this.markSession(FAILED, "Test " + tr.getName() + ": is fail");
        }
    }

    @Override
    public void onTestSkipped(ITestResult tr) {
        ConsoleLogger.log.info("Test '" + tr.getName() + "' SKIPPED");
        endTime = System.currentTimeMillis();
        setTimeStamp(tr.getName(), endTime - startTime);
        startTime = 0;
        endTime = 0;
        if (Boolean.parseBoolean(Prop.getField("bs.available"))) {
            this.markSession(FAILED, "Some test has been skipped, because the ".concat(failedCase).concat(" is fail"));
        }
    }
...
//                String instants = iTestNGMethod.getInstance().toString();
    ...
    private void markSession(String status, String reason) {
        ...
        if (Android.driver() != null) {
            jse = (JavascriptExecutor) Android.driver();
        } else if (Ios.driver() != null) {
            jse = (JavascriptExecutor) Ios.driver();
        } else {
            throw new RuntimeException("Driver is null");
        }
        ...
        if (status != null && reason != null)
            ...
            throw new RuntimeException("BS session is not marked! Body is not correct. Current status:" + status + " current reason:" + reason);
    }
}
